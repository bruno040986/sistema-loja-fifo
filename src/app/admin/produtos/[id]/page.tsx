...
export default function EditarProdutoPage({ params }: { params: Promise<{ id: string }> }) {
        ...
        if (data && !data.error) {
          ...
          if (data.batches && data.batches.length > 0) {
            setBatches(data.batches.map((b: any) => ({
              id: b.id,
              expirationDate: new Date(b.expirationDate).toISOString().split('T')[0],
              price: b.price.toString(),
              bulkPrice: b.bulkPrice ? b.bulkPrice.toString() : "",
              bulkQuantity: b.bulkQuantity ? b.bulkQuantity.toString() : "",
              stock: b.stock.toString(),
              isFeatured: b.isFeatured || false,
            })));
          } else {
            setBatches([{ expirationDate: "", price: "", bulkPrice: "", bulkQuantity: "", stock: "", isFeatured: false }]);
          }
        }
        setFetching(false);
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...formData,
      batches: batches.filter(b => b.expirationDate && b.price && b.stock)
    };
  ...
  if (fetching) return <div className="text-center p-12">Carregando dados do produto...</div>;
